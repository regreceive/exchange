...
export function thowErrorMaxCap(message) {
  return {
    type: 'EXCHANGE.THROW_SOURCE_AMOUNT_ERROR',
    payload: message,
  };
...
export function updateRateExchange(ethereum, source, dest, sourceAmount, sourceTokenSymbol, isManual = false) {
  return {
...
export function updateRateSnapshotComplete(rateInit, expectedPrice, slippagePrice) {
  ...
  return {
    ...
    payload: {
      rateInit,
      expectedPrice,
      slippagePrice: converter.toT(slippagePrice, 18),
      rateInitSlippage: converter.toT(rateInit, 18),
    },
  };
}
...
export function checkTokenBalanceOfColdWallet(
  formId,
  ethereum,
  address,
  sourceToken,
