    ...
    return Object.keys(this.props.display.listItem).map((key, i) => {
      return (
        <div key={key} onClick={e => this.selectDisplayColumn(e, key)} className="display-item">
          <i>icon</i>
          <label>{this.props.display.listItem[key]}</label>
        </div>
      );
    });
  };
    ...
    return Object.keys(this.props.shows.listItem).map((key, i) => {
      ...
      return (
        <label key={key} for={this.props.shows.listItem[key].title} className="column-label">
          <span className={checked ? 'item-title' : ''}>
            {this.props.translate(this.getTranslateFromKey(key)) || title}
          </span>
          <input
            id={this.props.shows.listItem[key].title}
            type="checkbox"
            onChange={e => this.selectShowsColumn(e, key)}
            checked={checked}
          />
