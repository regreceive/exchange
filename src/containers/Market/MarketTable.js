...
export default class MarketTable extends React.Component {
  ...
  handleScroll = () => {
    if (
      this.props.listTokens.length > this.props.firstPageSize &&
      !this.props.isLoading &&
      this.props.page - 1 < this.props.numScroll
    ) {
  ...
  drawChart = props => {
    ...
    return (
      <Line width={200} height={70} data={data} options={options} hideXAxis={true} hideYAxis={true} hidePoints={true} />
    );
  };
    ...
    if (input < 0) {
      return (
        <span className="negative">
          {input} %<img src={require('../../../assets/img/landing/arrow_red.svg')} />
        </span>
      );
    }
    if (input === 0) {
    ...
    if (input > 0) {
      return (
        <span className="positive">
          {input} %<img src={require('../../../assets/img/landing/arrow_green.svg')} />
        </span>
      );
    }
  };
    ...
    var columns = [
      {
        ...
        getHeaderProps: () => {
          return {
            className: this.props.sortType['market']
              ? this.props.sortType['market'] + ' -cursor-pointer'
              : '-cursor-pointer',
            onClick: e => {
      ...
      },
      {
        ...
        getHeaderProps: () => {
          return {
            className: this.props.sortType['sellPrice']
              ? this.props.sortType['sellPrice'] + ' -cursor-pointer'
              : '-cursor-pointer',
            onClick: e => {
      ...
      },
      {
        ...
        getHeaderProps: () => {
          return {
            className: this.props.sortType['buyPrice']
              ? this.props.sortType['buyPrice'] + ' -cursor-pointer'
              : '-cursor-pointer',
            onClick: e => {
            ...
            switch (key) {
              case 'change': {
                columns.push({
                  ...
                  getHeaderProps: () => {
                    return {
                      className: this.props.sortType[key]
                        ? this.props.sortType[key] + ' -cursor-pointer'
                        : '-cursor-pointer',
                      onClick: e => {
